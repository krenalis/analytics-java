package com.krenalis.analytics.internal;

import static com.krenalis.analytics.internal.FlushMessage.POISON;
import static com.krenalis.analytics.internal.StopMessage.STOP;
...
public class AnalyticsClientTest {
  ...
  public void batchRetriesForNetworkErrors() {
    ...
    verify(krenalisService, times(4)).upload(null, batch);
  ...
  public void batchDoesNotRetryForNon5xxAndNon429HTTPErrors() {
    ...
    verify(krenalisService).upload(null, batch);
  ...
  public void givesUpAfterMaxRetries() {
    ...
    verify(krenalisService, times(11)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
  ...
  public void shutdownWhenAlreadyShutDown() throws InterruptedException {
    ...
    verify(messageQueue, times(0)).put(any(Message.class));
    verifyNoInteractions(networkExecutor, callback, krenalisService);
  }
  ...
  public void neverRetries() {
    ...
    verify(krenalisService, times(1)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
